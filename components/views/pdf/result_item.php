...
<div id="res_<?= $item->internal_id ?>" class="panel">
    <div class="panel-heading">

        <!-- title -->
        <div id="res_<?= $item->internal_id ?>_t" class="<?= (!empty($item->retracted)) ? 'retraction-alert' : ''?>"
            ...
            ?>
            <?= Html::a(
                Yii::$app->bipstring->lowerize(Yii::$app->bipstring->shortenString($item->title, 180)),
                $url,
                    ...
                    if (empty($item->concepts))
                        ...
                        foreach ($item->concepts as $concept) { ?>
                            <span class="tag label">
                                ...
                                <?php
                                    $concept_score = round($concept['concept_score'], 2) * 100; // Convert score to percentage
                                ?>
                ...
                <div id="res_<?= $item->internal_id ?>_annot" class="tag-region grey-text">
                    ...
                    <?php foreach ($item->annotations as $annotation) { ?>
                        <span class="tag label">
                            ...
                            <span role="button" data-toggle="popover" data-placement="auto" title="<b><?= $annotation['label'] ?> </b>" data-content="<?= str_replace(['"', "\n", "\r"], ['&quot;', ' ', ' '], $annotation_content) ?>"><?= $annotation['label'] ?></span>
