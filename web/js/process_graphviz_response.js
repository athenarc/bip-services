 ...
 */
const delete_response = 'graph.removeCells(graph.getChildVertices(graph.getDefaultParent()));'; // Deletes all the nodes on the camvas
const node_array = [];
const edge_array = [];
...
function handle_response(response) { // The function that deletes the previous graph and shows the new one
    if (response.indexOf('DIMENSION') != -1) {
        const dim = response.split('DIMENSION');
        eval(dim[0]);
        response = dim[1];
    }
    ...
    eval(response);
    const dim_x = document.getElementById('graphviz_sub_parent').offsetWidth;
    const dim_y = document.getElementById('graphviz_sub_parent').offsetHeight;
    const cell_div_dimensions = graph.insertVertex(parent, 'div_dimen', '  ', dim_x, dim_y, 0, 0);
    const width = 120;
    const height = 60;
    const middle_node = 110;
