...
$(document).ready(() => {
    $('#Rag-form').submit(e => {
        e.preventDefault();
        const $ragInputs = $('#Rag-form :input');
        const ragValues = {};
        ...
        const startDate = Date.parse(ragValues.from_date);
        const endDate = Date.parse(ragValues.to_date);
                ...
                data:
            {
                ...
                success: function ({ found_date_period, new_rag_data }) {
                    if (found_date_period) {
                        ...
                        const newRag = $(`<tr id = rag_${new_rag_data.id}>`
    ...
    $(document).on('click', '.rag-delete', function () {
        const removedElement = $(this).closest('tr');
        const ragId = removedElement.attr('id').replace('rag_', '');
    ...
    $('#academic-age-datepicker-modal.edit_perm').on('hide.bs.modal', e => {
        const minYear = $('#academic-age-indicator').data('minYear');
        const academicAge = $('#academic-age-indicator').data('academicAge');
