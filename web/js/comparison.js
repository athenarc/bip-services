...
function updateComparisonMsg(mode) {
    ...
    if (articles_num != 0 || (articles_num == 0 && mode != 'init')) {
        ...
        if (articles_num == 1) {
            ...
            document.getElementById('clear-comparison').style.visibility = 'visible'; // show clear all button
            const pos = document.getElementById('comparison').className.search('disabled');
            if (pos != -1) { document.getElementById('comparison').className = document.getElementById('comparison').className.substring(0, pos); }
        }

        if (articles_num == 0) { setTimeout(() => { document.getElementById('comparison').style.visibility = 'hidden'; }, 3000); } // message to disappear after 3 sec
...
function highlightSelectedRows() {
    ...
    let cookie_contents;
    let selected_ids;
    let cookie_name; cookie_name = 'bipComparison';
    cookie_contents = getCookie(cookie_name);
    selected_ids = cookie_contents.split(',');
    $.each(selected_ids, (index, value) => {
        if (value != '') {
            let cur_id;
            cur_id = `res_${value}`;
            if (document.getElementById(cur_id) !== null) { highlightRow(cur_id); }
        }
    });
}
...
function clearSelected() {
    ...
    $.each(selected_ids, (index, value) => {
        if (value != '') {
            let cur_id;
            cur_id = `res_${value}`;
            if (document.getElementById(cur_id) !== null) { clearRow(cur_id); }
        }
    });
 ...
 * @author Thanasis Vergoulis
 */
$('.selected-after-click').click(event => {
    if (!event.target.id) { return; }

    const tokens = event.target.id.split('_');
    const row_id = `${tokens[0]}_${tokens[1]}`;
    const clicked_internal_article_id = tokens[1];
