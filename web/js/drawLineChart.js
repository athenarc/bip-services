...
function drawLineChart(min_x, max_x, max_y, data) {
    ...
    const svg = d3.select('.citation-line-chart').append('svg')
    ...
    const y = d3.scaleLinear().domain([0, max_y]).range([height - margin.top, margin.bottom]);

    // Define the axes
    const xAxis = d3.axisBottom().scale(x).tickFormat(d3.format('d')).ticks((max_x - min_x));
    ...
    const focusCircleArray = [];
    const focusCircleTextArray = [];
    ...
    if (colors.length === 0) {
        // Set chart legend
        const legend_keys = ['Selected Paper', 'Avg Exceptional Paper', 'Avg Substantial Paper'];
        ...
        lineLegend.append('text').text(d => d)
            .attr('transform', (d, i) =>
                `translate(${ 1.5 * margin.left + 200 * i },${ height + margin.top + margin.bottom / 4 })`);
        ...
        { // Add dynamically
            ...
            .attr('transform', (d, i) =>
                `translate(${ 1.5 * margin.left + 200 * i - 20 },${ height + margin.top + margin.bottom / 8 })`);
    }
}
