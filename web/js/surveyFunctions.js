...
function highlightSelectedRows() {
    const selected_papers_ids = getSelectedCheckboxIds();
    selected_papers_ids.forEach(paper_id => {
        const row_id = `res_${paper_id}`;
        highlightRow(row_id);
    });
}
...
function handleCheckboxClick(checkbox) {
    const row_id = `res_${checkbox.value}`;
...
function clearRow(row_id) {
    const pos = document.getElementById(row_id).className.search('success');
...
function getSelectedCheckboxIds() {
    return $.map($('input[name="selected_papers"]:checked'), (val, i) => val.value);
}
...
$('body').on('beforeSubmit', 'form#survey-form', function () {
    ...
    const selected_papers = getSelectedCheckboxIds();
