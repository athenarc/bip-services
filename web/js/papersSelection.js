$(document).ready(() => {
    function getSelectedPapersArray(listId) {
        const raw = $(`#selected_papers_${ listId}`).val();
    ...
    function checkSavedSelection(listId) {
        const selection = getSelectedPapersArray(listId);
        ...
        const total = $(`#select-works-modal-${ listId } .papers-selection-checkbox`).length;
        const checked = $(`#select-works-modal-${ listId } .papers-selection-checkbox:checked`).length;
    ...
    function getMaxAllowed(listId) {
        const modal = $(`#select-works-modal-${ listId}`);
    ...
    function enforceMaxState($modal) {
        const listId = $modal.attr('id').replace('select-works-modal-', '');
        const max = getMaxAllowed(listId);
        const $boxes = $modal.find('.papers-selection-checkbox');
        const $unchecked = $boxes.not(':checked');
        const $selectAll = $modal.find('.papers-select-on-check-all');
        ...
        const checkedCount = $modal.find('.papers-selection-checkbox:checked').length;
    ...
    function updateSelectAllLabel(listId) {
        const $modal = $(`#select-works-modal-${ listId}`);
        const total = $modal.find('.papers-selection-checkbox').length;
        const checked = $modal.find('.papers-selection-checkbox:checked').length;
        const $label = $modal.find('.select-all-toggle-label');

        // If all currently visible items are checked -> "Deselect All", else "Select All"
        const text = (total > 0 && checked === total) ? 'Deselect All' : 'Select All';
        $label.text(text);
    }

    function updateSelectionCounter(listId) {
        const max = getMaxAllowed(listId);
        const $badge = $(`#selection-counter-${ listId}`);
        ...
        const selected = $(`#select-works-modal-${ listId } .papers-selection-checkbox:checked`).length;
    ...
    function reconcileWithMax(listId) {
        ...
        const $modal = $(`#select-works-modal-${ listId}`);
        const $checked = $modal.find('.papers-selection-checkbox:checked');
    ...
    $('.modal').on('shown.bs.modal', function (e) {
        ...
        if (e && e.relatedTarget) {
            const maxFromButton = $(e.relatedTarget).data('max');
