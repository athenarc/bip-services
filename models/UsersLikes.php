...
class UsersLikes extends \yii\db\ActiveRecord {
    ...
    public static function getArticleLikePercentage($internal_id) {
        ...
        $paper_likes = self::find()->where(['showit' => true, 'paper_id' => $internal_id])->count();
    ...
    public static function UserHasLikes($user_id) {
        $has_likes = self::find()->where(['user_id' => $user_id, 'showit' => true])->exists();
    ...
    public static function updateTableValue($user_id, $paper_id, $column_name, $value) {
        $exists = self::find()->where(['user_id' => $user_id, 'paper_id' => $paper_id, 'showit' => true])->exists();

        if ($exists) {
            $user_like = self::find()->where(['user_id' => $user_id, 'paper_id' => $paper_id, 'showit' => true])->one();
