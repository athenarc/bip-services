...
class ResponsibleAcadAge extends \yii\db\ActiveRecord {
    ...
    public static function updateRag($user_orcid, $from_date, $to_date, $description) {
        $found = self::find()->where(['orcid' => $user_orcid, 'start_date' => $from_date, 'end_date' => $to_date])->exists();
        $saved_id = null;
        ...
        if (! $found) {
            ...
            $saved_rag = self::find()->where(['orcid' => $user_orcid, 'start_date' => $from_date, 'end_date' => $to_date])->one();
    ...
    public static function removeRag($rag_id) {
        $old_rag = self::find()->where(['id' => $rag_id])->one();
