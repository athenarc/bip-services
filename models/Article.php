...
namespace app\models;
    ...
    class Article extends \yii\db\ActiveRecord {
        ...
        public static function getPaperViewPercentage($internal_id) {
            $article = self::find()->where(['internal_id' => $internal_id])->one();
        ...
        public static function getJournalNamesAutoComplete($expansion, $max_num, $term) {
            ...
            if ($expansion == 'left') {
                $search_type = '%' . $term;
            } elseif ($expansion == 'right') {
                $search_type = $term . '%';
            } else {
                $search_type = '%' . $term . '%';
            }

            $journal_names = self::find()->select('journal')->distinct()->
        ...
        */
        ...
        public function calculatePyramidStatisticsTotal() {
            // count all articles
            $articles_total = self::find()->count();
            ...
            $this->pop_total = self::getTopPercentage($more_popular_total, $articles_total);
            $this->inf_total = self::getTopPercentage($more_influential_total, $articles_total);
        }
        ...
        public function calculatePyramidStatisticsJournal() {
            ...
            if ($this->journal) {
                ...
                // calculate percentages
                $this->inf_journal = self::getTopPercentage($more_influential_in_journal, $articles_in_journal);
                $this->pop_journal = self::getTopPercentage($more_popular_in_journal, $articles_in_journal);
                $this->imp_journal = self::getTopPercentage($more_impulsive_in_journal, $articles_in_journal);
                $this->cc_journal = self::getTopPercentage($more_cc_in_journal, $articles_in_journal);
            }
        }
        ...
        public function calculateCitationHistory() {
            $article_citation_history = self::findOne($this->internal_id)->getCitationHistory()->asArray()->all();
            ...
            foreach ($years_missing as $missing_year) {
                ...
                array_splice($article_citation_history, $missing_year_offset, 0, [['year' => $missing_year, 'cc' => 0]]);
            }
            ...
            array_push($article_citation_history, ['year' => $domain_max_x, 'cc' => 0]);

            //Dummy array of paper citation histories. Will only contain one history
            $this->citation_history = [$this->internal_id => $article_citation_history];
            ...
            array_push($avg_exceptional_paper_history, ['year' => $avg_exceptional_paper_history[count($avg_exceptional_paper_history) - 1]['year'], 'cc' => 0]);
            array_unshift($avg_exceptional_paper_history, ['year' => $domain_min_x, 'cc' => 0]);
            ...
            array_push($avg_substantial_paper_history, ['year' => $avg_substantial_paper_history[count($avg_substantial_paper_history) - 1]['year'], 'cc' => 0]);
            array_unshift($avg_substantial_paper_history, ['year' => $domain_min_x, 'cc' => 0]);
            //Get max of avg influential paper
        ...
        public function formatDetails() {
            ...
            if (trim($this->abstract) == '') {
                $this->abstract = 'N/A';
            } elseif (strlen($this->abstract) > 550) {
